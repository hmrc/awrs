...
import connectors.{TestSpikeAuthConnector, GovernmentGatewayAdminConnector}
import models.{ GsoAdminEnrolCredentialIdentifierXmlInput, Identifier}
...
// $COVERAGE-OFF$
trait TestSpikeEnrolService {

  val authConnector: TestSpikeAuthConnector

  def enrolAWRS(response: HttpResponse, safeId: String)(implicit headerCarrier: HeaderCarrier): Future[HttpResponse] =
    createEnrolment(response, safeId) flatMap authConnector.enrol
  ...
  def getCredIdentifier(implicit headerCarrier: HeaderCarrier): Future[String] = {
  ...
  object TestSpikeEnrolService extends TestSpikeEnrolService {
