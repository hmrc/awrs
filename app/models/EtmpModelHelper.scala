...
trait EtmpModelHelper extends EtmpConstants {
  ...
  def toEtmpGroupMember(groupMember: GroupMember): JsValue = {
    ...
    Json.obj(
      ...
      "address" -> toEtmpAddress(groupMember.address.reduceLeft((_, y) => y)),
      "identification" -> identification)
  }
    ...
    yestoTrue(placeOfBusiness.mainPlaceOfBusiness.fold("")(x => x)) match {
      case true =>
        ...
        placeOfBusiness.copy(mainAddress = Some(Address(postcode = businessAndAddress.postcode,
          ...
          addressLine4 = businessAndAddress.line_4))).mainAddress.reduceLeft((_, y) => y)
      case false => placeOfBusiness.mainAddress.reduceLeft((_, y) => y)
    }
  ...
  def toEtmpContactDetails(st: SubscriptionTypeFrontEnd): JsValue = {
    ...
    val address = businessDetails.contactAddressSame match {
      case Some("No") => Json.obj("address" -> toEtmpAddress(businessDetails.contactAddress.reduceLeft((_, y) => y)))
      case _ => Json.obj()
    }
  ...
  def toEtmpSupplierDetails(sp: Supplier): JsValue =
    Json.obj(
      ...
      .++(Json.obj("address" -> toEtmpAddress(sp.supplierAddress.reduceLeft((_, y) => y))))
  ...
  def toEtmpPartnerDetailsIndividual(partnerDetail: Partner): JsObject = {
    ...
    Json.obj(
      ...
      "partnerAddress" -> toEtmpAddress(partnerDetail.partnerAddress.reduceLeft((_, y) => y)),
