...
class SubscriptionControllerTest extends UnitSpec with OneServerPerSuite with MockitoSugar with AwrsTestJson {
  ...
  "SubscriptionController" must {
    ...
    "subscribe" must {
      ...
      "respond with OK" in {
        when(mockSubcriptionService.subscribe(Matchers.any(), Matchers.any())(Matchers.any())).thenReturn(Future.successful(registerSuccessResponse))
      ...
      "for an unsuccessful match return Not found" in {
        when(mockSubcriptionService.subscribe(Matchers.any(), Matchers.any())(Matchers.any())).thenReturn(Future.successful(matchFailureResponse))
        val result = TestSubscriptionController.subscribe("").apply(FakeRequest().withJsonBody(api4FrontendLTDJson))
      ...
      "for a bad request, return BadRequest" in {
        when(mockSubcriptionService.subscribe(Matchers.any(), Matchers.any())(Matchers.any())).thenReturn(Future.successful(HttpResponse(BAD_REQUEST, Some(badRequestJson))))
      ...
      "for service unavailable, return service unavailable" in {
        when(mockSubcriptionService.subscribe(Matchers.any(), Matchers.any())(Matchers.any())).thenReturn(Future.successful(HttpResponse(SERVICE_UNAVAILABLE, Some(serviceUnavailable))))
        val result = TestSubscriptionController.subscribe("").apply(FakeRequest().withJsonBody(api4FrontendLTDJson))
      ...
      "internal server error, return internal server error" in {
        when(mockSubcriptionService.subscribe(Matchers.any(), Matchers.any())(Matchers.any())).thenReturn(Future.successful(HttpResponse(INTERNAL_SERVER_ERROR, Some(serverError))))
        val result = TestSubscriptionController.subscribe("").apply(FakeRequest().withJsonBody(api4FrontendLTDJson))
